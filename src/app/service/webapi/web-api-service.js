...
var web_api_mock_content_1 = require("./web-api-mock-content");
var WebApiService = (function (_super) {
    __extends(WebApiService, _super);
    function WebApiService() {
        var _this = _super !== null && _super.apply(this, arguments) || this;
        _this.productReviews = [
            {
                id: 1,
                idProduct: 6280637,
                user: "Анастасия",
        ...
        _this.productPropValue1 = {
            ...
            prop_Value_Enum: _this.propEnumList1,
            prop_Value_Long: null,
            id_Measure_Unit: 8796097184356,
            idx: 0,
            out_bmask: 3
        };
        _this.productPropValue2 = {
            id: 2,
            id_Product: 6280637,
            id_Prop: _this.prop2,
            prop_Value_Str: null,
        ...
        _this.productPropValue4 = {
            ...
            prop_Value_Number: null,
            prop_Value_Bool: null,
            prop_Value_Enum: _this.propEnumList3,
            prop_Value_Long: null,
            id_Measure_Unit: 8796095644260,
            idx: 3,
            out_bmask: 3
        };
        _this.productPropValue5 = {
            id: 5,
            id_Product: 6280637,
            id_Prop: _this.prop3,
            prop_Value_Str: null,
            prop_Value_Number: null,
            prop_Value_Bool: null,
            prop_Value_Enum: _this.propEnumList4,
            prop_Value_Long: null,
            id_Measure_Unit: 8796095447652,
            idx: 3,
            out_bmask: 3
        };
        _this.productPropValue6 = {
            id: 6,
            id_Product: 6294898,
            id_Prop: _this.prop3,
            prop_Value_Str: null,
            prop_Value_Number: null,
            prop_Value_Bool: null,
            prop_Value_Enum: _this.propEnumList5,
            prop_Value_Long: null,
            id_Measure_Unit: 8796095644260,
            idx: 4,
            out_bmask: 3
        };
        _this.productPropValue7 = {
            id: 7,
            id_Product: 6293680,
            id_Prop: _this.prop1,
            prop_Value_Str: null,
        ...
        _this.productPropValue10 = {
            ...
            prop_Value_Number: null,
            prop_Value_Bool: null,
            prop_Value_Enum: _this.propEnumList6,
            prop_Value_Long: null,
        ...
        _this.clientOrderSpecProductsOfClient = [
            {
                idClient: 100,
                id: 6,
                idOrder: 3,
        ...
        _this.pages = [
            {
                id: 1,
                name: "home",
                content: _this.dynamicContent['HOME']
            }
        ];
        // </editor-fold>
        ...
        _this.actions = [
            {
                ...
                sketch_content: _this.dynamicContent['ACTION'],
                action_content: _this.dynamicContent['PARTSPAY']
            },
            {
                id: 2,
                ...
                sketch_content: _this.dynamicContent['ACTION'],
                action_content: _this.dynamicContent['ROUTERGIFT']
            }
        ];
        _this.actionOffers = [
            {
        ...
        _this.productSupplCreditGrades = [
        ...
        _this.novelties = [
            {
                ...
                priority: 10,
                sketch_content: _this.dynamicContent['NOVELTY'],
                novelty_content: _this.dynamicContent['WILEYFOXSPARK']
            },
            {
                id: 2,
                productId: 6280637,
                ...
                priority: 10,
                sketch_content: _this.dynamicContent['NOVELTY'],
                novelty_content: _this.dynamicContent['SAMSUNGGS8']
            }
        ];
        _this.noveltyDetails = [
            {
                id: 1,
                noveltyId: 1,
                productId: 6310491
        ...
        _this.userHandler = {
            post: function (info) { return null; },
            get: function (info) {
                var tokenStr = info.req.headers.get("Authorization");
                if (!_this.verifyToken(tokenStr)) {
                    return info.utils.createResponse$(function () {
            ...
            put: function (info, resOpt) {
                var user = info.req._body;
                var tokenStr = info.req.headers.get("Authorization");
                if (!user || !_this.verifyToken(tokenStr)) {
                    return info.utils.createResponse$(function () {
                        return new http_1.ResponseOptions({
                            status: 401,
                            statusText: "Unauthorized"
                ...
                var resultUser = _this.getEditBehavior(user);
                resOpt.body = resultUser;
                return info.utils.createResponse$(function () { return resOpt; });
            }
        };
        _this.clientPollAnswersHandler = {
            post: function (info) {
                var body = info.req._body;
                var tokenStr = info.req.headers.get("Authorization");
                var userId = info.req.headers.get("x-user");
                if (!userId || !_this.verifyToken(tokenStr)) {
                    return info.utils.createResponse$(function () { return new http_1.ResponseOptions({ status: 401, statusText: 'Unauthorized' }); });
                }
                if (!body || body.pollResult.length === 0) {
                    return info.utils.createResponse$(function () { return new http_1.ResponseOptions({ status: 400, statusText: 'Bad Request' }); });
                }
                var keys = Object.keys(body.pollResult);
                var lastclientAnswerId = _this.clientPollAnswers[_this.clientPollAnswers.length - 1].id;
                for (var i = 0; i < keys.length; i++) {
                    _this.clientPollAnswers.push({
                ...
                return info.utils.createResponse$(function () { return new http_1.ResponseOptions({
                    status: 201,
                    statusText: 'Created',
                    body: _this.clientPollAnswers[_this.clientPollAnswers.length - 1]
                }); });
            },
            get: function (info) { return null; }
        };
        return _this;
        // </editor-fold>
    }
    WebApiService.prototype.post = function (info) {
        var response;
        if ((response = this.apiController(info)) !== null)
            return response;
    };
    WebApiService.prototype.put = function (info) {
        var response;
        if ((response = this.apiController(info)) !== null)
            return response;
    };
    WebApiService.prototype.get = function (info) {
        var response;
        if ((response = this.apiController(info)) !== null)
            return response;
    };
    WebApiService.prototype.createDb = function () {
        var mquotationProducts = this.quotationProducts;
        var mproducts = this.products;
        var mquotation = this.quotations;
        var mcurrencies = this.currencies;
        var msuppliers = this.suppliers;
        var mproductReviews = this.productReviews;
        var manufacturers = this.manufacturers;
        var mcities = this.cities;
        var mstores = this.stores;
        var mtoken = this.tokens;
        var musers = this.users;
        var mproductStorePlaces = this.productStorePlaces;
        var mstorePlaces = this.storePlaces;
        var mlocalization = this.localization;
        var mclients = this.clients;
        var mcountries = this.countries;
        var mclientAddresses = this.clientAddresses;
        var mloEntities = this.loEntities;
        var mloSupplEntities = this.loSupplEntities;
        var mclientOrders = this.clientOrders;
        var mclientOrderSpecProducts = this.clientOrderSpecProducts;
        var mclientOrderSpecProductsOfClient = this.clientOrderSpecProductsOfClient;
        var mcartProducts = this.cartProducts;
        var mpages = this.pages;
        var mactions = this.actions;
        var mactionOffers = this.actionOffers;
        var mstoreReviews = this.storeReviews;
        var mpaymentMethods = this.paymentMethods;
        var mproductSupplCreditGrades = this.productSupplCreditGrades;
        var mcreditProducts = this.creditProducts;
        var mclientDraftOrder = this.clientDraftOrder;
        var mnovelties = this.novelties;
        var mpolls = this.polls;
        var mpollQuestion = this.pollQuestion;
        var mpollQuestionAnswer = this.pollQuestionAnswer;
        var mclientPollAnswers = this.clientPollAnswers;
        var mnoveltyDetails = this.noveltyDetails;
        var mclientBonuses = this.clientBonuses;
        var mpersons = this.persons;
        var mspecLOTrackingLog = this.specLOTrackingLog;
        var mcategories = this.categories;
        var mmeasureUnits = this.measureUnits;
        var mproductDescriptions = this.productDescriptions;
        var mProductImages = this.productImages;
        var mregions = this.regions;
        return {
            mquotationProducts: mquotationProducts,
            mproducts: mproducts,
            mquotation: mquotation,
            mcurrencies: mcurrencies,
            msuppliers: msuppliers,
            mproductReviews: mproductReviews,
            manufacturers: manufacturers,
            mcities: mcities,
            mstores: mstores,
            mtoken: mtoken,
            musers: musers,
            mproductStorePlaces: mproductStorePlaces,
            mstorePlaces: mstorePlaces,
            mlocalization: mlocalization,
            mclients: mclients,
            mcountries: mcountries,
            mclientAddresses: mclientAddresses,
            mloEntities: mloEntities,
            mloSupplEntities: mloSupplEntities,
            mclientOrders: mclientOrders,
            mclientOrderSpecProducts: mclientOrderSpecProducts,
            mclientOrderSpecProductsOfClient: mclientOrderSpecProductsOfClient,
            mcartProducts: mcartProducts,
            mpages: mpages,
            mactions: mactions,
            mactionOffers: mactionOffers,
            mstoreReviews: mstoreReviews,
            mpaymentMethods: mpaymentMethods,
            mproductSupplCreditGrades: mproductSupplCreditGrades,
            mcreditProducts: mcreditProducts,
            mclientDraftOrder: mclientDraftOrder,
            mnovelties: mnovelties,
            mpolls: mpolls,
            mpollQuestion: mpollQuestion,
            mpollQuestionAnswer: mpollQuestionAnswer,
            mclientPollAnswers: mclientPollAnswers,
            mnoveltyDetails: mnoveltyDetails,
            mclientBonuses: mclientBonuses,
            mpersons: mpersons,
            mspecLOTrackingLog: mspecLOTrackingLog,
            mcategories: mcategories,
            mmeasureUnits: mmeasureUnits,
            mproductDescriptions: mproductDescriptions,
            mProductImages: mProductImages,
            mregions: mregions
        };
    };
    // <editor-fold desc="monkey controller">
    WebApiService.prototype.apiController = function (info) {
        var resOpt = new http_1.ResponseOptions({
            status: 200,
            statusText: "OK",
            body: null
        });
        ...
        switch (info.collectionName) {
            case "mtoken": {
                var loginData = info.req._body;
                if (!loginData)
                    return info.utils.createResponse$(function () { return resOpt; });
                var loginModel = this.getTokenBehavior(loginData.phone, loginData.password);
            ...
            case "mgetDeliveryCost": {
                ...
                if (reqData.order.idStorePlace)
                    ...
                    if (reqData.order.idQuotationProduct === 7)
            ...
            case "mgetPromocodeDiscount": {
                ...
                if (reqData.promoCode) {
                    var numb = reqData.promoCode.match(/\d/g);
            ...
            case "mcalculateCart": {
                var reqData_1 = info.req._body;
                var _respDataArr_1 = [];
                reqData_1.cartContent.forEach(function (i) {
                    var _bonusDisc = (reqData_1.maxBonusCnt > 0) ? Math.round(i.price * 0.1) : null;
                    var _promoCodeDisc = (reqData_1.promoCode) ? Math.round(i.price * 0.05) : null;
                    var _promoBonusDisc = (reqData_1.usePromoBonus) ? Math.round(i.price * 0.03) : null;
                    _respDataArr_1.push({ clOrderSpecProdId: i.id, promoCodeDisc: _promoCodeDisc,
                        bonusDisc: _bonusDisc, promoBonusDisc: _promoBonusDisc });
                });
                resOpt.body = JSON.stringify(_respDataArr_1);
                return info.utils.createResponse$(function () { return resOpt; });
            }
    ...
    WebApiService.prototype.getTokenBehavior = function (email, password) {
        if (!email || !password)
            return null;
        var userResult = this.users.find(function (val) { return val.email === email && val.password === password; });
        return !userResult
            ? null
            : {
    ...
    WebApiService.prototype.getEditBehavior = function (user) {
        for (var i = 0; i < this.users.length; i++) {
            if (this.users[i].phone === user.phone) {
                ...
                if (user.appKey)
                    this.users[i].appKey = user.appKey;
                return this.users[i];
            }
        }
        return null;
    };
    WebApiService.prototype.verifyToken = function (token) {
        if (!token)
            return false;
        var tokenSplit = token.split(":");
        return (tokenSplit.length === 2 &&
            !!this.tokens.find(function (value) { return value.token === tokenSplit[1].trim(); }));
    };
    return WebApiService;
}(web_api_mock_content_1.WebApiMockContent));
